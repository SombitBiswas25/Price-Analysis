...
// Route: Get Commodities (Gold, Silver, Crude Oil) + Exchange rate
app.get('/api/commodities', async (req, res) => {
  ...
  try {
    const fetchPromises = Object.entries(tickers).map(async ([key, ticker]) => {
      ...
      // Clean null data from arrays
      const dataPoints = timestamps.map((timestamp, index) => ({
        time: timestamp * 1000, // convert to JS timestamp (ms)
    ...
    // Return mock fallback data so application never crashes
    const commodities = {};
...
// Route: Get Domestic Petrol prices in major Indian cities (Scraper + Fallback)
